...
# Extract workflow file path and referenced workflows
workflow_file=$(echo "$GITHUB_WORKFLOW_REF" | sed -E 's|^[^/]+/[^/]+/(.+)@.*|\1|')
referenced_workflows=$(curl -sL \
  -H "Accept: application/vnd.github.raw+json" \
  -H "Authorization: Bearer ${AUTH_TOKEN}" \
  "https://api.github.com/repos/${GITHUB_REPOSITORY}/actions/runs/${GITHUB_RUN_ID}" \
  | jq -c '[.referenced_workflows[]? | {path: .path, ref: .ref}]')

echo "Primary workflow file: $workflow_file"
echo "Referenced workflows: $referenced_workflows"
...
EOF
    ...
    issue_json=$(jq -n --arg title "$title" --arg body "$issue_body" \
      '{title: $title, body: $body, labels: ["security"], assignees: []}')

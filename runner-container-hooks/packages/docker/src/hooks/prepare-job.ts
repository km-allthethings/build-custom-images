...
async function runSetupRegistryAuth(): Promise<void> {
  try {
    ...
    // Assuming the script is at the root of the repository
    ...
    execSync(`bash ${scriptPath}`, {
      stdio: 'inherit', // This will show the script output in the runner logs
      env: {
        ...process.env,
    ...
    // Don't fail the job if auth setup fails - let it continue and fail naturally if needed
  }
}
